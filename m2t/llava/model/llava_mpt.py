...
class LlavaMPTModel(MPTModel):
    ...
    def forward(
        ...
        if (
            ...
            if isinstance(images, list):
                image_features = [
                    self.mm_projector(image_feature)[0] for image_feature in image_features
                ]
            else:
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                ...
                if vision_tower.config.use_im_start_end:
                    ...
                    if (cur_input_ids == vision_tower.config.im_start_token).sum() != (
                        ...
                        raise ValueError(
                            "The number of image start tokens and image end "
                    ...
                    if (cur_input_ids == vision_tower.config.im_patch_token).sum() != num_patches:
                        raise ValueError(
                            "The number of image patch tokens should be the same as the "
            ...
            if pretrain_mm_mlp_adapter:
                ...
                if input_embeddings.shape == embed_tokens_weight.shape:
                    ...
                    raise ValueError(
                        ...
                        f"{embed_tokens_weight.shape}. Current: {input_embeddings.shape}."
                        f" Numer of new tokens: {num_new_tokens}."
                    )

        vision_config.im_patch_token = tokenizer.convert_tokens_to_ids([DEFAULT_IMAGE_PATCH_TOKEN])[
            0
        ]
