    ...
    **kwargs,
    ...
    if isinstance(module, nn.Linear):
        ...
        if emb_init_std is not None:
            ...
            if verbose > 1:
                warnings.warn(
                    "Embedding layer initialized using normal distribution "
                    + f"with mean=0 and std={std!r}."
                )
...
def baseline_param_init_fn_(
    ...
    **kwargs,
    ...
    if init_std is None:
        raise ValueError(
            "You must set model.init_config['init_std'] to a float value to use the "
...
def xavier_uniform_param_init_fn_(
    ...
    **kwargs,
    ...
    if verbose > 1:
        warnings.warn(
            "Using torch.nn.init.xavier_uniform_ init fn with parameters: " + f"gain={init_gain}"
        )
    generic_param_init_fn_(
        module=module,
...
def xavier_normal_param_init_fn_(
    ...
    **kwargs,
    ...
    if verbose > 1:
        warnings.warn(
            "Using torch.nn.init.xavier_normal_ init fn with parameters: " + f"gain={init_gain}"
        )
    generic_param_init_fn_(
        module=module,
