    ...
    def forward(
        ...
        if self.training:
            if self.attn_uses_sequence_id and sequence_id is None:
                raise ValueError(
                    "sequence_id is a required argument when MPT is configured with "
                    + "attn_uses_sequence_id=True "
                    + "and the model is in train mode."
                )
            elif self.attn_uses_sequence_id is False and sequence_id is not None:
                warnings.warn(
                    "MPT received non-None input for `sequence_id` but is configured "
                    + "with attn_uses_sequence_id=False. "
                    + "This input will be ignored. If you want the model to use "
                    + "`sequence_id`, set attn_uses_sequence_id to True."
                )
        if input_ids is not None:
            S = input_ids.size(1)
            assert S <= self.config.max_seq_len, (
                f"Cannot forward input with seq_len={S}, this model only supports "
                + f"seq_len<={self.config.max_seq_len}"
            )
        ...
        if self.alibi:
            ...
            if S + past_position > self.config.max_seq_len:
                raise ValueError(
                    f"Cannot forward input with past sequence length {past_position} and "
                    "current sequence length {S + 1}, this model only supports total "
